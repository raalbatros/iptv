...
export default {
  async fetch(request, env, ctx) {
    ...
    // EDİTÖR — her seferinde GitHub'dan taze çeker
    if (url.pathname === "/editor") {
      try {
        const html = await fetch(
          `https://raw.githubusercontent.com/raalbatros/iptv/refs/heads/main/index.html?t=${Date.now()}`,
        ...
        return new Response("Editör yüklenemedi: " + e.message, { status: 500 });
      }
    }
        ...
        try {
          ...
          if (res.ok) {
            return new Response(await res.text(), {
              headers: { "Content-Type": "application/xml", "Access-Control-Allow-Origin": "*", "Cache-Control": "public, max-age=7200" }
            });
          }
        } catch(e) { continue; }
      }
    ...
    if (action === "get_live_categories") {
      return jsonResponse([{ category_id: "1", category_name: "📺 Canlı TV", parent_id: 0 }]);
    }
    ...
    if (action === "get_series_categories") {
      return jsonResponse([{ category_id: "10", category_name: "📺 Diziler", parent_id: 0 }]);
    }
    ...
    if (action === "get_series_info") {
      try {
        ...
        return jsonResponse({
          info: { name: d.name, cover: d.cover || "", plot: d.overview || "", rating: "8" },
          seasons,
          episodes
        });
      } catch(e) {
    ...
    if (action === "get_simple_data_table") {
      return jsonResponse({ epg_url: `${url.origin}/xmltv.xml`, tv_archive: 0, tv_archive_duration: 0 });
    }

    if (!action) {
      return jsonResponse({
        user_info: {
          auth: 1, status: "Active", exp_date: "1893456000",
          username: CONFIG.USERNAME, password: CONFIG.PASSWORD,
          is_trial: "0", active_cons: "0", created_at: "1609459200",
          max_connections: "1", allowed_output_formats: ["m3u8", "ts", "rtmp"]
        },
        server_info: {
          url: url.hostname, port: "80", https_port: "443",
...
function parseLiveM3U(m3uText, categoryId, baseUrlOrigin) {
  ...
  const result = [];
  let streamId = 1;
  let currentName = "", currentLogo = "", currentEpgId = "";
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.match(/^(https?|rtmp|rtsp|udp|rtp):\/\//i)) {
      result.push({
        stream_id: streamId, num: streamId,
        name: currentName || `Kanal ${streamId}`,
        stream_type: "live", type: "live",
        category_id: categoryId,
        stream_icon: currentLogo, thumbnail: currentLogo,
        ...
        tv_archive: 0, tv_archive_duration: 0, is_adult: "0",
        added: Math.floor(Date.now() / 1000).toString()
      });
      streamId++;
      currentName = ""; currentLogo = ""; currentEpgId = "";
    }
  }
  return result;
}
...
function extractShortEPG(epgXml, channelId) {
  ...
  while ((match = regex.exec(epgXml)) !== null && programmes.length < 5) {
    if (match[1] === channelId) {
      programmes.push({ channel_id: match[1], start: match[2], end: match[3], title: match[4], description: "", genre: "", genre_id: "" });
    }
  }
  return programmes;
}
...
function parseSimpleM3U(m3uText, categoryId, baseUrlOrigin, idOffset = 0) {
  ...
  const result = [];
  let localId = 1;
  let currentName = "", currentLogo = "", pendingInfo = false;
  for (let i = 0; i < lines.length; i++) {
    ...
    if (pendingInfo && line.match(/^https?:\/\//i)) {
      ...
      result.push({
        stream_id: globalId, num: globalId, name: currentName,
        stream_type: "movie", category_id: categoryId,
        stream_icon: currentLogo, rating_5based: "0",
        stream_url: `${baseUrlOrigin}/movie/${CONFIG.USERNAME}/${CONFIG.PASSWORD}/${globalId}.mp4`,
        direct_source: line, container_extension: "mp4",
        added: Math.floor(Date.now() / 1000).toString(),
  ...
  for (const line of lines) {
    ...
    if (pendingInfo && trimmed.match(/^https?:\/\//i)) {
      ...
      result.push({
        stream_id: globalId, num: globalId, name: currentName,
        stream_type: "movie", category_id: categoryId,
        stream_icon: currentLogo, rating_5based: currentRating,
        stream_url: `${baseUrlOrigin}/movie/${CONFIG.USERNAME}/${CONFIG.PASSWORD}/${globalId}.mp4`,
        direct_source: trimmed, container_extension: "mp4",
        added: Math.floor(Date.now() / 1000).toString(),
